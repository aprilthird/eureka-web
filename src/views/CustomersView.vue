...
<template>
  ...
  <v-sheet class="mb-5 pa-4 overflow-hidden">
    <v-container>
      <v-row>
        <v-col>
          <v-text-field v-model="query" label="Nombre" variant="outlined"></v-text-field>
        </v-col>
      </v-row>
    </v-container>
  ...
  <div class="mb-2 font-weight-bold">Clientes</div>
  ...
  <v-dialog width="500" v-model="removeDialog">
    <template v-slot:default="{ isActive }">
      <v-card title="Desactivar Cliente">
...
export default {
  ...
  methods: {
    fetchData() {
      this.loading = true
      fetch(`${ApiUtils.customersUrl}?q=${this.query}`, {
        headers: {
    ...
    edit() {
      ...
      fetch(`${ApiUtils.customersUrl}/${this.id}`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${this.authResponse.accessToken}`,
          'Content-Type': 'application/json'
